...
#define SIGMA_MIN (0.01) //MACROS argc/argv ?
...
vlc_module_begin ()
    ...
    add_float_with_range( FILTER_PREFIX "sigma", 2., SIGMA_MIN, SIGMA_MAX, //PARAMETRE ???
...
// //REECRIRE TOUT CA AVEC  10 par exemple
...
//     for( int x = -i_dim; x <= i_dim; x++ ) //POUR LA TAILLE DU FLOU
//     {
//         //FORMULE QUI CREE LE FLOU // TESTER CHANGER VAL
//         const float f_distribution = sqrt( exp(-(x*x)/(f_sigma*f_sigma) ) / (2.*M_PI*f_sigma*f_sigma) );
// #ifdef DONT_USE_FLOATS
//         const float f_factor = 1 << 8; //FLOAT EN INT ??? PQ decalage binaire ?
// #else
//         const float f_factor = 1;
// #endif

//         pt_distribution[i_dim+x] = (type_t)( f_distribution * f_factor ); // PQ [index] ???
...
static void gaussianblur_InitDistribution( filter_sys_t *p_sys )
{
    double f_sigma = p_sys->f_sigma;
    int i_dim = (int)(3.*10);
    type_t *pt_distribution = xmalloc( (2*30+1) * sizeof( type_t ) );

    for( int x = -30; x <= 30; x++ ) //POUR LA TAILLE DU FLOU
    {
        //FORMULE QUI CREE LE FLOU // TESTER CHANGER VAL
        const float f_distribution = sqrt( exp(-(x*x)/(10*10) ) / (2.*M_PI*10*10) );
#ifdef DONT_USE_FLOATS
        const float f_factor = 1 << 8; //FLOAT EN INT ??? PQ decalage binaire ?
#else
        const float f_factor = 1;
...
static int Create( vlc_object_t *p_this ) // CREATE filter + gestion d'erreur des parametre envoyé (return statement)
{
    filter_t *p_filter = (filter_t *)p_this; // initialize p_filter
    ...
    p_filter->p_sys = malloc( sizeof( filter_sys_t ) );              //one if pas de {} ????
    ...
    p_filter->pf_video_filter = Filter;  // PAS DE PARAMETRE ??
...
static void Destroy( vlc_object_t *p_this ) // Destroy filter
{
...
// Change image with filter
static picture_t *Filter( filter_t *p_filter, picture_t *p_pic )   //p_pic ???
{
    picture_t *p_outpic;
    ...
    if( !p_pic ) return NULL;                                    //PAS DE FILTRE
                     ...
                     y++ )
                {
                         ...
                         x++ )
                    {
                        t_value += pt_distribution[y+i_dim] *
                                   pt_distribution[x+i_dim];  //DISTRIBUTION ??? côté rectangle ????
                    }
                }
